...
class UtilV2View(discord.ui.LayoutView):
    def __init__(self):
        super().__init__(timeout=None)
        container = discord.ui.Container(id=1, accent_color=0x2b2d31)
        container.add_item(discord.ui.TextDisplay("**[편의기능]**"))
        container.add_item(discord.ui.TextDisplay("🗡️ 강화 및 공/방 계산"))
        row1 = discord.ui.ActionRow()
        b1 = discord.ui.Button(label="카프라스 계산기", style=discord.ButtonStyle.success, custom_id="util_cap")
        b1.callback = self.on_cap; row1.add_item(b1)
        b2 = discord.ui.Button(label="공/방 구간 계산기", style=discord.ButtonStyle.danger, custom_id="util_apdp")
        b2.callback = self.on_apdp; row1.add_item(b2)
        b3 = discord.ui.Button(label="포식 계산기", style=discord.ButtonStyle.primary, custom_id="util_devour")
        b3.callback = self.on_devour; row1.add_item(b3)
        b4 = discord.ui.Button(label="군왕 무기 계산기", style=discord.ButtonStyle.secondary, custom_id="util_sov")
        b4.callback = self.on_sov; row1.add_item(b4)
        container.add_item(row1)
        container.add_item(discord.ui.Separator())
        container.add_item(discord.ui.TextDisplay("💰 펄의상 및 거래소 효율 계산"))
        row2 = discord.ui.ActionRow()
        for label, cid, cb in [("데키아 등불","util_dekia",self.on_dekia), ("영물의 축복","util_bless",self.on_bless), ("펄 의상","util_pearl",self.on_pearl), ("거래소 실수령액","util_tax",self.on_tax), ("아이템 평균가","util_avg",self.on_avg)]:
            b = discord.ui.Button(label=label, style=discord.ButtonStyle.secondary, custom_id=cid)
            b.callback = cb; row2.add_item(b)
        container.add_item(row2)
        container.add_item(discord.ui.Separator())
        container.add_item(discord.ui.TextDisplay("📋 기타"))
        row3 = discord.ui.ActionRow()
        b5 = discord.ui.Button(label="어둠의 틈", style=discord.ButtonStyle.secondary, custom_id="util_rift")
        b5.callback = self.on_rift; row3.add_item(b5)
        b6 = discord.ui.Button(label="저격 수렵 계산기", style=discord.ButtonStyle.secondary, custom_id="util_snipe")
        b6.callback = self.on_snipe; row3.add_item(b6)
        b7 = discord.ui.Button(label="제작노트 🔗", style=discord.ButtonStyle.secondary, custom_id="util_craftnote")
        b7.callback = self.on_craft; row3.add_item(b7)
        container.add_item(row3)
        self.add_item(container)

    async def on_cap(self, i: discord.Interaction):
        await i.response.send_message(embed=discord.Embed(title="카프라스 계산기", description="아래 버튼을 클릭해주세요.", color=0x2ecc71), view=CaphrasSelectView(), ephemeral=True); schedule_ephemeral_delete(i)
    async def on_apdp(self, i: discord.Interaction):
        await i.response.send_modal(ApDpModal())
    async def on_devour(self, i: discord.Interaction):
        await i.response.send_message(embed=discord.Embed(title="포식 계산기", description="아래 버튼을 클릭해주세요.", color=0x2b2d31), view=DevourSelectView(), ephemeral=True); schedule_ephemeral_delete(i)
    async def on_sov(self, i: discord.Interaction):
        await i.response.send_message(embed=discord.Embed(title="군왕 무기 계산기", description="부위를 선택해주세요.", color=0x2b2d31), view=SovWeaponSelectView(), ephemeral=True); schedule_ephemeral_delete(i)
    async def on_dekia(self, i: discord.Interaction):
        await i.response.defer(ephemeral=True); schedule_ephemeral_delete(i)
        tasks = [get_market_price(it["id"], it["sid"]) for it in DEKIA_DB]; res = await asyncio.gather(*tasks)
        rk = sorted([{"n":it["name"], "l":it["light"], "p":p, "s":s, "u":p//it["light"]} for it, (p,s,_) in zip(DEKIA_DB, res) if p>0], key=lambda x:x["u"])
        d = "".join([f"**{idx+1}. {r['n']}**\n불빛: {r['l']} | 가격: {r['p']:,}\n**1개당: `{r['u']:,}`**\n\n" for idx, r in enumerate(rk)])
        await i.followup.send(embed=discord.Embed(title="데키아 랭킹", description=d[:4000] or "데이터 없음", color=0xf1c40f), ephemeral=True)
    async def on_bless(self, i: discord.Interaction):
        await i.response.defer(ephemeral=True); schedule_ephemeral_delete(i)
        tasks = [get_market_price(it["id"]) for it in BLESS_DB]; res = await asyncio.gather(*tasks)
        rk = sorted([{"n":it["name"], "r":it["residue"], "p":p, "s":s, "u":p//it["residue"]} for it, (p,s,_) in zip(BLESS_DB, res) if p>0], key=lambda x:x["u"])
        d = "".join([f"**{idx+1}. {r['n']}**\n잔재: {r['r']} | 가격: {r['p']:,}\n**1개당: `{r['u']:,}`**\n\n" for idx, r in enumerate(rk)])
        await i.followup.send(embed=discord.Embed(title="영물 랭킹", description=d[:4000] or "데이터 없음", color=0x2ecc71), ephemeral=True)
    async def on_pearl(self, i: discord.Interaction):
        await i.response.send_message(embed=discord.Embed(title="펄 의상", color=0x2b2d31), view=PearlTimeView(), ephemeral=True); schedule_ephemeral_delete(i)
    async def on_tax(self, i: discord.Interaction):
        await i.response.send_message(embed=discord.Embed(title="거래소 실수령액", color=0x2b2d31), view=TaxSelectView(), ephemeral=True); schedule_ephemeral_delete(i)
    async def on_avg(self, i: discord.Interaction):
        await i.response.send_modal(AvgPriceModal())
    async def on_rift(self, i: discord.Interaction):
        conn = sqlite3.connect('bdo_data.db'); row = conn.execute("SELECT kill_time, reporter_name FROM rift_history WHERE id=1").fetchone(); conn.close()
        if row: e = build_rift_status_embed(datetime.strptime(row[0], '%Y-%m-%d %H:%M:%S').replace(tzinfo=KST) + timedelta(days=5), row[1])
        else: e = discord.Embed(title="어둠의 틈", description="기록 없음.", color=0x2b2d31)
        await i.response.send_message(embed=e, view=DarkRiftActionView(), ephemeral=True); schedule_ephemeral_delete(i)
    async def on_snipe(self, i: discord.Interaction):
        await i.response.send_message(embed=discord.Embed(title="저격 수렵", color=0x2b2d31), view=SnipeMainView(), ephemeral=True); schedule_ephemeral_delete(i)
    async def on_craft(self, i: discord.Interaction):
        await i.response.send_message("https://bdocodex.com/kr/recipe/", ephemeral=True); schedule_ephemeral_delete(i)
...
async def cmd_install_util(interaction: discord.Interaction):
    ...
    await interaction.channel.send(view=UtilV2View())
...
async def setup_hook():
    ...
    # bot.add_view(OfficialLinksView())  # 링크 버튼은 persistent 불가
